...
//
// Ограничение: если на логгере открыта группа (WithGroup), идентификатор
// попадёт внутрь неё — «delivery.request_id» вместо «request_id», — потому что
// группы применяет вложенный обработчик, а поле добавляется к записи. Вынести
// его наружу можно, только заново реализовав семантику групп slog поверх
// накопленных атрибутов; ради пути, которым проект не пользуется (WithGroup
// нигде не вызывается), эта сложность не окупается. Если группы понадобятся,
// начинать нужно отсюда — поведение зафиксировано тестом.
